 ...
 * Module  : Building DSP bus 
 *         : DSPバスの構築
 * File    : attach_dsp_setting.c
 *
 ****************************************************************************/
...
static const CriChar8 *snapshot_name[NUM_SNAPSHOT] = {NULL, "Snapshot1", "Snapshot2"};
...
void app_display_info(AppObj *app_obj)
{
	...
	/* Display title */
	line_no = 2;
	criFwPrt_DrawText(2, line_no++, "CRI ADX2 Sample : Attach DspBusSetting");
	...
	criFwPrt_DrawText(3, line_no++, "DSP Snapshot   : %s", ((snapshot_name[app_obj->snapshot_index] == NULL) ? "DEFAULT" : snapshot_name[app_obj->snapshot_index]));
	...
	criFwPrt_DrawText( 3, line_no, "%s", criFwPad_GetButtonName(CRIFWPAD_BUTTON05));
	criFwPrt_DrawText(12, line_no++, "Apply Next Snapshot");
...
void Sample_Initialize(void)
{
	...
	/* DSPバス設定は初期化処理内で適用される */
	/* The DSP bus setting is applied within the initialization process. */
	...
	/* スナップショット番号の初期化 */
	/* Initialize DSP snapshot number */
	app_obj->snapshot_index = 0;
	
	/* DSP BUS1を通して再生するよう設定 */
	/* Configure for playback through DSP BUS1 */
	criAtomExPlayer_SetBusSendLevelOffset(app_obj->player, 1, app_obj->bus_send_level);
...
CriBool Sample_Update(void)
{
	...
	if (pad_status->down & CRIFWPAD_BUTTON05) {
		if (++app_obj->snapshot_index >= NUM_SNAPSHOT) {
			app_obj->snapshot_index = 0;
		}
		criAtomEx_ApplyDspBusSnapshot(snapshot_name[app_obj->snapshot_index], 3000);
	}
