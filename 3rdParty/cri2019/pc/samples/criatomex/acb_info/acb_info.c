 ...
 * Module  : Acquiring ACB information
 *         : ACB情報の取得
 * File    : acb_info.c
 *
 ****************************************************************************/
...
void app_display_info(AppObj *app_obj)
{
	...
	const CriChar8 *format_string[] = {"NONE", "ADX", "AHX", "HCA", "HCA MX", "WAVE"};
	CriAtomExCueId cue_id;
	const CriChar8* cue_name;
	const CriChar8* user_data;
	CriSint64 length, ms, num_samples;
	...
	/* ACB情報の表示 */
	/* Display ACB info */
	criFwPrt_DrawText(1, i++, "[ACB info sample]");
	...
	i = 4;
	criFwPrt_DrawText(40, i++, "[Waveform Info]");
	criFwPrt_DrawText(40, i++, "%-16s:%d", "Wave ID", waveform_info->wave_id);
	if (waveform_info->format <= CRIATOMEX_FORMAT_WAVE) {
		criFwPrt_DrawText(40, i++, "%-16s:%s", "Format", format_string[waveform_info->format]);
	} else {
		criFwPrt_DrawText(40, i++, "%-16s:%s", "Format", "(OTHER)");
	}
	criFwPrt_DrawText(40, i++, "%-16s:%d", "Sampling Rate", waveform_info->sampling_rate);
	criFwPrt_DrawText(40, i++, "%-16s:%d", "Num Channels", waveform_info->num_channels);
	criFwPrt_DrawText(40, i++, "%-16s:%lld", "Num Samples", waveform_info->num_samples);
	criFwPrt_DrawText(40, i++, "%-16s:%s", "Streaming Flag", (waveform_info->streaming_flag ? "TRUE" : "FALSE"));
	i++;

	/* ステータスの表示 */
	/* Display status */
	player_status = criAtomExPlayer_GetStatus(app_obj->player);
	criFwPrt_DrawText(1, i++, "Player Status   :%s", player_status_string[player_status]);
	playback_status = criAtomExPlayback_GetStatus(app_obj->playback_id);
	criFwPrt_DrawText(1, i++, "Playback Status :%s", playback_status_string[playback_status]);
	i++;

	/* 再生時刻／再生サンプル数の表示 */
	/* Display playback time */
	ms = criAtomExPlayer_GetTime(app_obj->player);
	criFwPrt_DrawText(1, i++, "Player Time     :%lld [ms]", ms);
	criAtomExPlayback_GetNumPlayedSamples(app_obj->playback_id, &num_samples, NULL);
	criFwPrt_DrawText(1, i++, "Played samples  :%lld", num_samples);
	i++;
