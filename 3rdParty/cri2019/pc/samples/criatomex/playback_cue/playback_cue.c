...
void app_display_info(AppObj *app_obj)
{
	...
	i = 0;
	criFwPrt_DrawText(1, i++, "[Playback cue sample]");
...
void Sample_Initialize(void)
{
	...
	/* 再生するキューのIDを初期化 */
	/* Initialize Cue ID for playback */
	app_obj->cue_id = 0;
}
...
CriBool Sample_Update(void)
{
	...
	if (pad_status->down & CRIFWPAD_UP) {
		app_obj->cue_id++;
		if (app_obj->cue_id > CRI_ATOMCUESHEET_VOICE_C) {
			app_obj->cue_id = 0;
		}
	}
	if (pad_status->down & CRIFWPAD_DOWN) {
		app_obj->cue_id--;
		if (app_obj->cue_id < 0) {
			app_obj->cue_id = CRI_ATOMCUESHEET_VOICE_C;
		}
	}
