 ...
 * File    : acb_info_with_in_game_preview.c
 *
 ****************************************************************************/
...
#define IN_GAME_ACF_FILE				"inGamePreview/InGame.acf"
#define ACB_FILE						"inGamePreview/InGame.acb"
...
void app_display_info(AppObj *app_obj)
{
	...
	CriBool is_connected;

	i = 0;
	/* ACB情報の表示 */
	/* Display ACB info */
	criFwPrt_DrawText(1, i++, "[ACB info with in game preview sample]");

	/* Toolとの接続状態を取得 */
	/* Get the status of connection to tool */
	is_connected = criAtomExMonitor_IsConnected();
	if ( criAtomExMonitor_IsConnected() ) {
		if ( criAtomExMonitor_GetClientIpString() != CRI_NULL ) {
			criFwPrt_DrawText( 1, i++, "Connection Status: %s (Client IP:%s)", (is_connected) ? "CONNECTED" : "AcceptWaiting", criAtomExMonitor_GetClientIpString());
		} else {
			criFwPrt_DrawText( 1, i++, "Connection Status: %s",  (is_connected) ? "CONNECTED" : "AcceptWaiting");
		}
	} else {
		if ( criAtomExMonitor_GetServerIpString() != CRI_NULL ) {
			criFwPrt_DrawText( 1, i++, "Connection Status: %s (Server IP:%s)", (is_connected) ? "CONNECTED" : "AcceptWaiting", criAtomExMonitor_GetServerIpString());
		} else {
			criFwPrt_DrawText( 1, i++, "Connection Status: Status: %s",  (is_connected) ? "CONNECTED" : "AcceptWaiting");
		}
	}
	...
	if (cue_id >= 0) {
		...
		criFwPrt_DrawText(1, i++, "%-16s:%s", "User Data", (user_data ? user_data : ""));
		if (length >= 0) {
...
void Sample_Initialize(void)
{
	...
	/* アセットのパスを取得 */
	/* Get the asset path */
	smputl_get_asset_path(acf_path, IN_GAME_ACF_FILE, sizeof(acf_path));
	...
	/* モニタライブラリの初期化 */
	/* Initialization of monitor library */
