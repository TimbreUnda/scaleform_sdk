 ...
 * Module  : Sample for mainloop with 30fps
 *         : ゲームループ30fps
 * File    : game_loop_30fps.c
 *
 ****************************************************************************/
...
void app_display_info(AppObj *app_obj)
{
	...
	i = 1;
	criFwPrt_DrawText(1, i++, "[Game loop 30fps sample]");
	criFwPrt_DrawText(1, i++, "Server Freq  :30.0f");
...
void Sample_Initialize(void)
{
	...
	/* サーバ動作周波数を30fpsに変更 */
	/* Change server frequency to 30fps */
	ex_config.server_frequency = 30.0f;
	...
	/* パフォーマンスモニタ機能の追加 */
	/* Add the performance monitor feature */
...
CriBool Sample_Update(void)
{
	...
	/* ゲームループ30fpsを模擬するため、２度目のV同期を実行する */
	/* 2nd Vsync to simulate 30fps game loop */
