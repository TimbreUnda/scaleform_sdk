...
void CriSmpSoundOutputXAudio2::ExecuteAudioFrame(void)
{
	...
	for (j = 0; j < wsmpl; j++) {
		for (n = 0; n < NUM_CHANNELS; n++) {
			pcmbuf[j * NUM_CHANNELS + n] = sample[n][j];
		}
	}
...
#endif
{
	...
	CriUint32 start_time;
	CriUint32 end_time;
	CriSint32 sleep_time;
	
	start_time = sndout->GetTime();
	while (sndout->thread_exit_flag == CRI_FALSE) {
		...
		end_time = sndout->GetTime();
		if (end_time >= start_time) {
			sleep_time = (CriSint32)AUDIO_FRAME_INTERVAL - (CriSint32)(end_time - start_time);
		} else {
			sleep_time = 0;
		}
		start_time = sndout->GetTime();
...
CriUint32 CriSmpSoundOutputXAudio2::GetTime()
{
#if defined(_XBOX)
	return (CriUint32)GetTickCount();
#else
	return (CriUint32)timeGetTime();
#endif
}
