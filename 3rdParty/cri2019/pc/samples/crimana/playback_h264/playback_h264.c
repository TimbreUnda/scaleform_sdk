 ...
 * Module  : Main function.(Playing H264)
 *
 * Description :
 *  H.264コーデックのムービを再生するサンプルです。
 *  ヘッダ解析無し。ムービ解像度は固定。ワークバッファはメモリ関数登録。
 *
 * Description :
 *  This is a sample program that plays a movie of H.264 codec.
...
#define USM_FILE		PATH_MANA "1280x720_2997fps_h264.usm"
...
/* デフォルトのデバイス解像度（ウィンドウ枠のサイズ） */
#define DEFAULT_DEVICE_WIDTH	(640)
#define DEFAULT_DEVICE_HEIGHT	(480)
...
void app_display_info(AppObj *app_obj)
{
	...
	x = 2; y = 1;
	criFwPrt_DrawText(x, y, "[CRI Mana: Playback H.264]"); y++;
...
CriBool app_mana_initialize(AppObj *app_obj)
{
	...
	CriManaMediaFoundationH264DecoderConfig_PC h264_config;
	...
	/* H.264デコーダの初期化 (必ずManaの初期化の前に行ってください) */
	/* Initialize the Media Foundation H.264 video decoder (Be sure to do this before the Mana initialization) */
	criMana_SetDefaultMediaFoundationH264DecoderConfig_PC(h264_config);
	criMana_SetupMediaFoundationH264Decoder_PC(&h264_config, NULL, 0);
	...
	if ((DEFAULT_DEVICE_WIDTH < MOVIE_WIDTH) || (DEFAULT_DEVICE_HEIGHT < MOVIE_HEIGHT)) {
		/* デバイス解像度の設定 * /
		/* ウィンドウ枠のサイズをデフォルトの640x480からリサイズ */
		/* Set device resolution  */
		smpvideo_set_device_resolution(MOVIE_WIDTH, MOVIE_HEIGHT);
	}
