/*
	CRI File System Tutorial No.2 (Load data from CPK file)

	Copyright (c) 2008-2009 CRI Middleware Co., Ltd.

（１）プログラムの内容
  このプログラムは、パッキングされたCPKファイルからデータをロードするサンプルです。
...
int criSample_Main(int argc, char *argv[])
{
	...
	/* ファイルサイズの取得 */
	criFsBinder_GetFileSize(binder, DATA_FILE_NAME, &fsize);
	/* ファイル読み込み用バッファの確保 */
	bufsize = ROUND_UP(fsize + 1, 32);
	buffer = (CriUint8*)criSmpFsUtl_Alloc((CriUint32)bufsize);
	/* ファイルのロード */
	criFsLoader_Load(loader, binder, DATA_FILE_NAME, 0, fsize, buffer, bufsize);
